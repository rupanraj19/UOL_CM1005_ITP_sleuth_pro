...
Officer: 8681181
CaseNum: 802-3-30045669-8681181
...
- First write a shuffleSeed() function. Same as before, it needs to return an array of 52 random integers between 4 and 97.
...
- This should search for card matching cut_point inside playingCards.
- Once you've found a match use the JavaScript splice() function to store the cut card and all the elements after from the playingCards array in the top_of_deck array. 
- You'll need to break the loop once you've done as you don't want to keep searching.
- Finally reverse the elements in top_of_deck so that the card we cut the deck at is the last element and not the first. 
...
var cut_point = { card_suit: 'Clubs', value: 'J'} ;
var top_of_deck = [];
...
function deck_cut(){
	...
	for(var i =0; i < playingCards.length; i++){
		if(playingCards[i].suit == cut_point.card_suit && playingCards[i].no == cut_point.value){
			preReversal = playingCards.splice(i, playingCards.length -1);
			break;
		}
	}
	for(var i = preReversal.length; i > 0; i--){
		top_of_deck.push(preReversal[i-1]);
	}
}
